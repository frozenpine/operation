...
class WSSHBridge(object):
    ...
    def __init__(self, websocket, commandbuffer):
    ...
    def open(
        ...
        try:
            ...
            self._websocket.send(json.dumps({
                'error': 'Could not resolve hostname {0}: {1}'.format(
                    hostname, e.args[1]
                )
            }))
            raise
    ...
    def _forward_inbound(self, channel):
        ...
        try:
            while True:
                ...
                if 'data' in data:
                    channel.send(
                        self._commandbuffer.handle_key(
                            data['data']
    ...
    def _forward_outbound(self, channel):
        ...
        try:
            ...
            while True:
                ...
                recv = channel.recv(self._windowsize)
                if len(recv) <= 0:
                    return
                try:
                    ...
                    response = recv.decode('gbk', 'ignore')
                    self._decoder = 'gbk'
                data += self._commandbuffer.handle_response(response)
                try:
                    self._websocket.send(json.dumps({
                        'data': data,
